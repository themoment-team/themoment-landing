...
/* The form itself, and the only client component on the page's second half.
   The send happens here rather than in a server action because Web3Forms
   refuses server-side callers outright — see the note in api/sendContact.
   Which is also why the key arrives as a prop: it is read from the
   environment by the section above, and has to reach the browser to be of
   any use.

   The comp has no submit button — three fields and nothing to press. So the
   button is built in the page's own language: the one blue on the page,
   inverting to white on hover the way the links do, and the width of the
   form it belongs to. */
export default function ContactForm({ accessKey }: { accessKey: string | null }) {
  const [state, setState] = useState(INITIAL_CONTACT_STATE);
  const [pending, setPending] = useState(false);
  const [values, setValues] = useState(EMPTY);

  /* When the form was first touched, for the too-fast check. Kept in a ref
     rather than state: it is not something the page renders, and holding it
     in state would re-render on every keystroke for nothing. */
  ...
  const handleChange =
    (key: FieldKey) => (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      if (!startedAt.current) startedAt.current = Date.now();
      setValues((prev) => ({ ...prev, [key]: event.target.value }));
    };

  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    /* The browser has already run the fields' own validation by the time a
       submit event fires, so this only ever sees a form worth sending. */
    event.preventDefault();
    if (pending) return;

    const form = event.currentTarget;
    setPending(true);
    const result = await sendContact(accessKey, {
      ...values,
      /* Read off the form rather than held in state — nothing renders it,
         and nobody who can see the page can have filled it. */
  ...
  /* These are controlled inputs, so they are cleared here — along with the
     message box's measured height, which would otherwise stay at the size of
     a letter that has already been sent. */
  useEffect(() => {
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="mt-block flex w-full max-w-[600px] flex-col gap-stack">
      {FIELDS.map((field, i) => {
          ...
          <Reveal key={field.key} delay={beat(i, GROUP)} className="flex w-full flex-col gap-2">
            ...
            {field.type === "textarea" ? (
              <textarea
                ...
                /* overflow-hidden goes with the growing: the box is always
                   as tall as its content, so there is nothing to scroll, and
                   a bar flickering in on the line that overflows would undo
                   the point of it. resize-none for the same reason — a
                   handle would be a second answer to a question the box
                   already answers. */
      ...
      {/* The honeypot. Pushed off-screen rather than display:none — there
          are bots that skip hidden fields and fill everything else. Nobody
          who can see the page can reach it, so anything in it came from
          something that was not looking. */}
      ...
      <Reveal delay={beat(FIELDS.length, GROUP)} className="flex flex-col gap-3">
        <button
          type="submit"
          /* Off until there is something to send. sendContact validates
             again — this is only so the button stops inviting a press that
        ...
        {/* One line for every outcome, held in the live region the whole
            ...
            never announce. Failures are set in white rather than the brand
            blue so success and failure do not read alike at a glance; the
            words carry it either way, for anyone who cannot tell the two
            colours apart. */}
        <p
          role="status"
          aria-live="polite"
